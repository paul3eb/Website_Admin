...
use App\Models\Planning\Elementary;
...
class ElementaryController extends Controller
{
    ...
    {
        $elementaries = DB::table('elementaries')->orderBy('id','desc')->paginate(10);
        return view('planning.elementary.index', compact('elementaries'));
    }
    ...
    public function create(Request $request)
    {
        if($request->ajax())
        {
            ...
            $elementaries = DB::table('elementaries')->where('id', 'like', '%'.$search.'%')
                                                ->orWhere('name_school', 'like', '%'.$search.'%')
                                                ->orderBy($sort_by, $sort_type)
                                                ->paginate(10);
            return view('planning.elementary.partials.form', compact('elementaries'));
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasfile('file'))
        {
            ...
            $file->move('planning/elementary/', $filename);
            $data->file = $filename;
    ...
    public function update(Request $request)
    {
        ...
        $elementaries = Elementary::findOrfail($request->id);
        if($request->hasfile('edit_file'))
        {
            $destination = 'planning/elementary/'.$elementaries->file;
    ...
    public function destroy(Request $request)
    {
        $id = $request->id;
        $data = Elementary::find($id); 
        $destination = 'planning/elementary/'.$data->file; 
        if(File::exists($destination))
        {
            File::delete($destination);
        }
