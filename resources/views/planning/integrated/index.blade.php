...
<div id="AddIntegratedModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Integrated School</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="AddIntegratedForm" class="form-horizontal" >
                @csrf
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="add_Integrated_btn"class="add_Integrated btn btn-primary">
...
<div id="editIntegratedModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Edit Integrated School</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="editIntegratedForm" class="form-horizontal" >
              @method('PATCH')
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="edit_Integrated_btn"class="edit_Integrated btn btn-primary">
        ...
        <div class="row">
          <div class="col-lg-12">
              <div class="card">
                  ...
                  <a href="#" class="btn btn-lg btn-success" role="button" data-bs-toggle="modal" data-bs-target="#AddIntegratedModal">Add Integrated School</a> <br /><br />
                      ...
                      <table class="table table-sm " id="table">
                          <thead>
                              <tr>
                                ...
                                <th class="sorting" data-sorting_type="desc" style="cursor:pointer"data-column_name="title">School Name
                          ...
                          <tbody>
                            @include('planning.integrated.partials.form')
                          </tbody>
                      </table>
...
<script>
    $(document).ready(function(){
        ...
        function fetch_data(page, sort_type, sort_by, search="") { 
            $.ajax({
            url: '{{ route('integrated.create') }}?page='+page+'&sortby='+sort_by+'&sorttype='+sort_type+'&search='+search,
        ...
        // add new data ajax request.
            $("#AddIntegratedForm").submit(function(e) {
                    e.preventDefault();
                    var column_name = $('#hidden_column_name').val();
                    var sort_type = $('#hidden_sort_type').val();
                    var page = $('#hidden_page').val();
                    const fd = new FormData(this);
                    $("#add_Integrated_btn").text('');
                    $.ajax({
                      url: '{{ route('integrated.store') }}',
                      method: 'POST',
                      data: fd,
                      cache: false,
                      contentType: false,
                      ...
                      success: function(response) {
                        if (response.status == 1) {
                          Swal.fire(
                            'Added!',
                            'Integrated School Added Successfully!',
                            'success'
                          )
                          fetch_data(page, sort_type, column_name);
                        }
                        $("#add_Integrated_btn").text('Add Integrated School');
                        $("#AddIntegratedForm")[0].reset();
                        $("#AddIntegratedModal").modal('hide');
      ...
      $(document).on('click', '.editIcon', function(e) {
        ...
        var id = $(this).attr('id');
        $.ajax({
          url: '{{ route('integrated.edit','$data->id') }}',
          method: 'get',
          data: {
            id: id,
            _token: '{{ csrf_token() }}'
          ...
          success: function(response) {
            ...
            $("#edit_pdf").html(
              `<iframe src="/planning/integrated/${response.file}" width="100" class="img-fluid img-thumbnail" > </iframe>`);
            $("#id").val(response.id);
      ...
      $("#editIntegratedForm").submit(function(e) {
        e.preventDefault();
        var column_name = $('#hidden_column_name').val();
        var sort_type = $('#hidden_sort_type').val();
        var page = $('#hidden_page').val();
        const fd = new FormData(this);
        $.ajax({
          url: '{{ route('integrated.update','$integrateds->id') }}',
          method: "POST",
          data: fd,
          cache: false,
          contentType: false,
          ...
          success: function(response) {
            if (response.status == 1) {
              Swal.fire(
                'Updated!',
                'Integrated School Updated Successfully!',
                'success'
              )
              fetch_data(page, sort_type, column_name);
            }
            $("#edit_Integrated_btn").text('Update Integrated School');
            $("#editIntegratedForm")[0].reset();
            $("#editIntegratedModal").modal('hide');
          }
        });
      });
      ...
      $(document).on('click', '.deleteIcon', function(e) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '{{ route('integrated.destroy','$data->id') }}',
              method: 'delete',
              data: {
                id: id,
                _token: csrf
