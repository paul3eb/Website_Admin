...
<div id="AddSecondaryModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Secondary School</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="AddSecondaryForm" class="form-horizontal" >
                @csrf
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="add_Secondary_btn"class="add_Secondary btn btn-primary">
...
<div id="editSecondaryModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Edit Secondary School</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="editSecondaryForm" class="form-horizontal" >
              @method('PATCH')
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="edit_Secondary_btn"class="edit_Secondary btn btn-primary">
      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Secondary School</li>
          </ol>
        </div>
      </div>
    </div>
        ...
        <div class="row">
          <div class="col-lg-12">
              <div class="card">
                  ...
                  <a href="#" class="btn btn-lg btn-success" role="button" data-bs-toggle="modal" data-bs-target="#AddSecondaryModal">Add Secondary School</a> <br /><br />
                  ...
                  <div class=" card-body p-0" id="datatable">
                      ...
                      <table class="table table-sm " id="table">
                          ...
                          <tbody>
                            @include('planning.secondary.partials.form')
                          </tbody>
                      </table>
...
<script>
    $(document).ready(function(){
        ...
        function fetch_data(page, sort_type, sort_by, search="") { 
            $.ajax({
            url: '{{ route('secondary.create') }}?page='+page+'&sortby='+sort_by+'&sorttype='+sort_type+'&search='+search,
        ...
        // add new data ajax request.
            $("#AddSecondaryForm").submit(function(e) {
                    e.preventDefault();
                    var column_name = $('#hidden_column_name').val();
                    var sort_type = $('#hidden_sort_type').val();
                    var page = $('#hidden_page').val();
                    const fd = new FormData(this);
                    $("#add_Secondary_btn").text('');
                    $.ajax({
                      url: '{{ route('secondary.store') }}',
                      method: 'POST',
                      data: fd,
                      cache: false,
                      contentType: false,
                      ...
                      success: function(response) {
                        if (response.status == 1) {
                          Swal.fire(
                            'Added!',
                            'Secondary School Added Successfully!',
                            'success'
                          )
                          fetch_data(page, sort_type, column_name);
                        }
                        $("#add_Secondary_btn").text('Add Secondary School');
                        $("#AddSecondaryForm")[0].reset();
                        $("#AddSecondaryModal").modal('hide');
      ...
      $(document).on('click', '.editIcon', function(e) {
        ...
        var id = $(this).attr('id');
        $.ajax({
          url: '{{ route('secondary.edit','$data->id') }}',
          method: 'get',
          data: {
            id: id,
            _token: '{{ csrf_token() }}'
          ...
          success: function(response) {
            ...
            $("#edit_pdf").html(
              `<iframe src="/planning/secondry/${response.file}" width="100" class="img-fluid img-thumbnail" > </iframe>`);
            $("#id").val(response.id);
      ...
      $("#editSecondaryForm").submit(function(e) {
        e.preventDefault();
        var column_name = $('#hidden_column_name').val();
        var sort_type = $('#hidden_sort_type').val();
        var page = $('#hidden_page').val();
        const fd = new FormData(this);
        $.ajax({
          url: '{{ route('secondary.update','$secondaries->id') }}',
          method: "POST",
          data: fd,
          cache: false,
          contentType: false,
          ...
          success: function(response) {
            if (response.status == 1) {
              Swal.fire(
                'Updated!',
                'Secondary School Updated Successfully!',
                'success'
              )
              fetch_data(page, sort_type, column_name);
            }
            $("#edit_Secondary_btn").text('Update');
            $("#editSecondaryForm")[0].reset();
            $("#editSecondaryModal").modal('hide');
          }
        });
      });
      ...
      $(document).on('click', '.deleteIcon', function(e) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '{{ route('secondary.destroy','$data->id') }}',
              method: 'delete',
              data: {
                id: id,
                _token: csrf
