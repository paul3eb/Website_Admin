...
<div id="AddInvitationModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Invitation to Bid</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="AddInvitationForm" class="form-horizontal" >
                @csrf
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="add_invitation_btn"class="add_invitation btn btn-primary">
...
<div id="editInvitationModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Edit Invitation to Bid</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="editInvitationForm" class="form-horizontal" >
              @method('PATCH')
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="edit_invitation_btn"class="add_invitation btn btn-primary">
      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Invitation to Bid</li>
          </ol>
        </div>
      </div>
    </div>
        ...
        <div class="row">
          <div class="col-lg-12">
              <div class="card">
                  ...
                  <a href="#" class="btn btn-lg btn-success" role="button" data-bs-toggle="modal" data-bs-target="#AddInvitationModal">Add Invitation to Bid</a> <br /><br />
                  ...
                  <div class=" card-body p-0" id="datatable">
                      ...
                      <table class="table table-sm " id="table">
                          ...
                          <tbody>
                            @include('bac.invitation.partials.form')
                          </tbody>
                      </table>
...
<script>
    $(document).ready(function(){
        ...
        function fetch_data(page, sort_type, sort_by, search="") { 
            $.ajax({
            url: '{{ route('invitation.create') }}?page='+page+'&sortby='+sort_by+'&sorttype='+sort_type+'&search='+search,
        ...
        // add new data ajax request.
            $("#AddInvitationForm").submit(function(e) {
                    e.preventDefault();
                    var column_name = $('#hidden_column_name').val();
                    var sort_type = $('#hidden_sort_type').val();
                    var page = $('#hidden_page').val();
                    const fd = new FormData(this);
                    $("#add_invitation_btn").text('');
                    $.ajax({
                      ...
                      success: function(response) {
                        ...
                        $("#add_invitation_btn").text('Add Invitation');
                        $("#AddInvitationForm")[0].reset();
                        $("#AddInvitationModal").modal('hide');
      ...
      $(document).on('click', '.editIcon', function(e) {
        ...
        var id = $(this).attr('id');
        $.ajax({
          url: '{{ route('invitation.edit','$data->id') }}',
          method: 'get',
          data: {
            id: id,
            _token: '{{ csrf_token() }}'
          ...
          success: function(response) {
            ...
            $("#edit_pdf").html(
              `<iframe src="/bac/invitation/${response.file}" width="100" class="img-fluid img-thumbnail" > </iframe>`);
            $("#id").val(response.id);
      ...
      $("#editInvitationForm").submit(function(e) {
        e.preventDefault();
        var column_name = $('#hidden_column_name').val();
        var sort_type = $('#hidden_sort_type').val();
        var page = $('#hidden_page').val();
        const fd = new FormData(this);
        $.ajax({
          url: '{{ route('invitation.update','$invitation->id') }}',
          method: "POST",
          data: fd,
          cache: false,
          contentType: false,
          ...
          success: function(response) {
            ...
            $("#edit_invitation_btn").text('Update Invitation');
            $("#editInvitationForm")[0].reset();
      ...
      $(document).on('click', '.deleteIcon', function(e) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '{{ route('invitation.destroy','$data->id') }}',
              method: 'delete',
              data: {
                id: id,
                _token: csrf
