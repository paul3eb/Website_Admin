...
<div id="AddProceedModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Notice to Proceed</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="AddProceedForm" class="form-horizontal" >
                @csrf
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="add_Proceed_btn"class="add_Proceed btn btn-primary">
...
<div id="editProceedModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Edit Notice to Proceed</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="editProceedForm" class="form-horizontal" >
              @method('PATCH')
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="edit_Proceed_btn"class="edit_Proceed btn btn-primary">
      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Notice to Proceed</li>
          </ol>
        </div>
      </div>
    </div>
        ...
        <div class="row">
          <div class="col-lg-12">
              <div class="card">
                  ...
                  <a href="#" class="btn btn-lg btn-success" role="button" data-bs-toggle="modal" data-bs-target="#AddProceedModal">Add Notice to Proceed</a> <br /><br />
...
<script>
    $(document).ready(function(){
        ...
        function fetch_data(page, sort_type, sort_by, search="") { 
            $.ajax({
            url: '{{ route('proceed.create') }}?page='+page+'&sortby='+sort_by+'&sorttype='+sort_type+'&search='+search,
        ...
        // add new data ajax request.
            $("#AddProceedForm").submit(function(e) {
                    e.preventDefault();
                    var column_name = $('#hidden_column_name').val();
                    var sort_type = $('#hidden_sort_type').val();
                    var page = $('#hidden_page').val();
                    const fd = new FormData(this);
                    $("#add_Proceed_btn").text('');
                    $.ajax({
                      url: '{{ route('proceed.store') }}',
                      method: 'POST',
                      data: fd,
                      cache: false,
                      contentType: false,
                      ...
                      success: function(response) {
                        if (response.status == 1) {
                          Swal.fire(
                            'Added!',
                            'Proceed Added Successfully!',
                            'success'
                          )
                          fetch_data(page, sort_type, column_name);
                        }
                        $("#add_Proceed_btn").text('Add Proceed');
                        $("#AddProceedForm")[0].reset();
                        $("#AddProceedModal").modal('hide');
      ...
      $(document).on('click', '.editIcon', function(e) {
        ...
        var id = $(this).attr('id');
        $.ajax({
          url: '{{ route('proceed.edit','$data->id') }}',
          method: 'get',
          data: {
            id: id,
            _token: '{{ csrf_token() }}'
          ...
          success: function(response) {
            ...
            $("#edit_pdf").html(
              `<iframe src="/bac/proceed/${response.file}" width="100" class="img-fluid img-thumbnail" > </iframe>`);
            $("#id").val(response.id);
      ...
      $("#editProceedForm").submit(function(e) {
        e.preventDefault();
        var column_name = $('#hidden_column_name').val();
        var sort_type = $('#hidden_sort_type').val();
        var page = $('#hidden_page').val();
        const fd = new FormData(this);
        $.ajax({
          url: '{{ route('proceed.update','$proceeds->id') }}',
          method: "POST",
          data: fd,
          cache: false,
          contentType: false,
          ...
          success: function(response) {
            if (response.status == 1) {
              Swal.fire(
                'Updated!',
                'Proceed Updated Successfully!',
                'success'
              )
              fetch_data(page, sort_type, column_name);
            }
            $("#edit_Proceed_btn").text('Update Proceed');
            $("#editProceedForm")[0].reset();
            $("#editProceedModal").modal('hide');
          }
        });
      });
      ...
      $(document).on('click', '.deleteIcon', function(e) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '{{ route('proceed.destroy','$data->id') }}',
              method: 'delete',
              data: {
                id: id,
                _token: csrf
