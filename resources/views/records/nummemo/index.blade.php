...
<div id="AddNummemoModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Numbered Memorandum</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="AddNummemoForm" class="form-horizontal" >
                @csrf
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="add_Nummemo_btn"class="add_Nummemo btn btn-primary">
...
<div id="editNummemoModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Edit Numbered Memorandum</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="editNummemoForm" class="form-horizontal" >
              @method('PATCH')
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="edit_Nummemo_btn"class="add_Nummemo btn btn-primary">
        ...
        <div class="row">
          <div class="col-lg-12">
              <div class="card">
                  ...
                  <a href="#" class="btn btn-lg btn-success" role="button" data-bs-toggle="modal" data-bs-target="#AddNummemoModal">Add Numbered Memorandum</a> <br /><br />
                  ...
                  <div class=" card-body p-0" id="datatable">
                      ...
                      <table class="table table-sm " id="table">
                          ...
                          <tbody>
                            @include('records.nummemo.partials.form')
                          </tbody>
                      </table>
...
<script>
    $(document).ready(function(){
        ...
        function fetch_data(page, sort_type, sort_by, search="") { 
            $.ajax({
            url: '{{ route('memo.create') }}?page='+page+'&sortby='+sort_by+'&sorttype='+sort_type+'&search='+search,
        ...
        // add new data ajax request.
            $("#AddNummemoForm").submit(function(e) {
                    e.preventDefault();
                    var column_name = $('#hidden_column_name').val();
                    var sort_type = $('#hidden_sort_type').val();
                    var page = $('#hidden_page').val();
                    const fd = new FormData(this);
                    $("#add_Nummemo_btn").text('');
                    $.ajax({
                      ...
                      success: function(response) {
                        if (response.status == 1) {
                          Swal.fire(
                            'Added!',
                            'Numbered Memorandum Added Successfully!',
                            'success'
                          )
                          fetch_data(page, sort_type, column_name);
                        }
                        $("#add_Nummemo_btn").text('Add Numbered Memorandum');
                        $("#AddNummemoForm")[0].reset();
                        $("#AddNummemorModal").modal('hide');
      ...
      $(document).on('click', '.editIcon', function(e) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            $("#edit_pdf").html(
              `<iframe src="/records/nummemo/${response.file}" width="100" class="img-fluid img-thumbnail" > </iframe>`);
            $("#id").val(response.id);
      ...
      $("#editNummemoForm").submit(function(e) {
        e.preventDefault();
        var column_name = $('#hidden_column_name').val();
        var sort_type = $('#hidden_sort_type').val();
        var page = $('#hidden_page').val();
        const fd = new FormData(this);
        $.ajax({
          url: '{{ route('memo.update','$num_memos->id') }}',
          method: "POST",
          data: fd,
          cache: false,
          contentType: false,
          ...
          success: function(response) {
            if (response.status == 1) {
              Swal.fire(
                'Updated!',
                'Numbered Memorandum Updated Successfully!',
                'success'
              )
              fetch_data(page, sort_type, column_name);
            }
            $("#edit_Nummemo_btn").text('Update Numbered Memorandum');
            $("#editNummemoForm")[0].reset();
            $("#editNummemoModal").modal('hide');
          }
        });
      });
      ...
      $(document).on('click', '.deleteIcon', function(e) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '{{ route('memo.destroy','$data->id') }}',
              method: 'delete',
              data: {
                id: id,
                _token: csrf
