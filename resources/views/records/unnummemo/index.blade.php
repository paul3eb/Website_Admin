...
<div id="AddUnnummemoModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Add Unnumbered Memorandum</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="AddUnnummemoForm" class="form-horizontal" >
                @csrf
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="add_Unnummemo_btn"class="add_Unnummemo btn btn-primary">
...
<div id="editUnnummemoModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Edit Unnumbered Memorandum</h4>
                <button type="submit" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                
            </div>
            <form  method="POST" enctype="multipart/form-data" id="editUnnummemoForm" class="form-horizontal" >
              @method('PATCH')
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="submit" id="edit_Unnummemo_btn"class="add_Unnummemo btn btn-primary">
        ...
        <div class="row">
          <div class="col-lg-12">
              <div class="card">
                  ...
                  <a href="#" class="btn btn-lg btn-success" role="button" data-bs-toggle="modal" data-bs-target="#AddUnnummemoModal">Add Unnumbered Memorandum</a> <br /><br />
                      ...
                      <table class="table table-sm " id="table">
                          <thead>
                              <tr>
                                ...
                                <th class="sorting" data-sorting_type="desc" data-column_name="id" style="cursor:pointer">Date Uploaded
                          ...
                          <tbody>
                            @include('records.unnummemo.partials.form')
                          </tbody>
                      </table>
...
<script>
    $(document).ready(function(){
        ...
        function fetch_data(page, sort_type, sort_by, search="") { 
            $.ajax({
            url: '{{ route('unnummemo.create') }}?page='+page+'&sortby='+sort_by+'&sorttype='+sort_type+'&search='+search,
        ...
        // add new data ajax request.
            $("#AddUnnummemoForm").submit(function(e) {
                    e.preventDefault();
                    var column_name = $('#hidden_column_name').val();
                    var sort_type = $('#hidden_sort_type').val();
                    var page = $('#hidden_page').val();
                    const fd = new FormData(this);
                    $("#add_Unnummemo_btn").text('');
                    $.ajax({
                      url: '{{ route('unnummemo.store') }}',
                      method: 'POST',
                      data: fd,
                      cache: false,
                      contentType: false,
                      ...
                      success: function(response) {
                        if (response.status == 1) {
                          Swal.fire(
                            'Added!',
                            'Unnumbered Memorandum Added Successfully!',
                            'success'
                          )
                          fetch_data(page, sort_type, column_name);
                        }
                        $("#add_Unnummemo_btn").text('Add Unnumbered Memorandum');
                        $("#AddUnnummemoForm")[0].reset();
                        $("#AddUnnummemoModal").modal('hide');
      ...
      $(document).on('click', '.editIcon', function(e) {
        ...
        var id = $(this).attr('id');
        $.ajax({
          url: '{{ route('unnummemo.edit','$data->id') }}',
          method: 'get',
          data: {
            id: id,
            _token: '{{ csrf_token() }}'
          ...
          success: function(response) {
            ...
            $("#edit_pdf").html(
              `<iframe src="/records/unnummemo/${response.file}" width="100" class="img-fluid img-thumbnail" > </iframe>`);
            $("#id").val(response.id);
      ...
      $("#editUnnummemoForm").submit(function(e) {
        e.preventDefault();
        var column_name = $('#hidden_column_name').val();
        var sort_type = $('#hidden_sort_type').val();
        var page = $('#hidden_page').val();
        const fd = new FormData(this);
        $.ajax({
          url: '{{ route('unnummemo.update','$order->id') }}',
          method: "POST",
          data: fd,
          cache: false,
          contentType: false,
          ...
          success: function(response) {
            if (response.status == 1) {
              Swal.fire(
                'Updated!',
                'Unnumbered Memorandum Updated Successfully!',
                'success'
              )
              fetch_data(page, sort_type, column_name);
            }
            $("#edit_Unnummemo_btn").text('Update Unnumbered Memorandum');
            $("#editUnnummemoForm")[0].reset();
            $("#editUnnummemoModal").modal('hide');
          }
        });
      });
      ...
      $(document).on('click', '.deleteIcon', function(e) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '{{ route('unnummemo.destroy','$data->id') }}',
              method: 'delete',
              data: {
                id: id,
                _token: csrf
